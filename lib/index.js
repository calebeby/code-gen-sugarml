...
function walk (tree, options, indent=0, multiline=false) {
  ...
  return tree.reduce((m, node) => {
    ...
    // text node type
    if (node.type === 'text') {
      const doctype = /<!doctype (.*)>/i.exec(node.content)
      if (doctype) {
        return m + `doctype ${doctype[1]}`
      }
    ...
    if (node.type === 'tag') {
      ...
      if (selfClosing.indexOf(node.name) > -1) {
        ...
        if (node.content.filter(n => n.type === 'tag').length > 0) {
          childIndent++
          multiline = true
        } else {
          m += ' '
        }
        m += walk(node.content, options, childIndent, multiline)
      }
...
function attributes (options, obj) {
  let m = ''
  if (obj.id && obj.id.filter(id => id.type === 'text').length > 0) {
    m += '#' + obj.id.map(id => id.content).join('')
    delete obj.id
  }
  if (obj.class && obj.class.filter(c => c.type === 'text').length > 0) {
    m += '.' + obj.class.map(c => c.content).filter(c => c !== ' ').join('.')
    delete obj.class
  }
  if (obj && Object.keys(obj).length > 0) {
    m += '(' + innerAttributes(options, obj) + ')'
  }
  return m
}

// part of attributes within parens
function innerAttributes (options, obj) {
