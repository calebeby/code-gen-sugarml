    ...
    if (node.type === 'tag') {
      ...
      if (selfClosing.indexOf(node.name) > -1) {
        ...
        if (node.content) {
          let subindent = 0
          let multiline = false
          if (node.content.filter(subnode => subnode.type === 'tag').length > 0) {
            indent++
            multiline = true
          } else {
            m += ' '
          }
          m += walk(node.content, options, indent, multiline)
        }
      }
...
function attributes (options, obj) {
  let m = ''
  if (obj.id && obj.id.type === 'text') {
    m += '#' + obj.id.content
    delete obj.id
  }
  if (obj.class && obj.class.type === 'text') {
    m += '.' + obj.class.content.replace(' ', '.')
    delete obj.class
  }
